  ...
  return (
    <Card className="p-4">
          ...
          {marketInfo ? (
            <>
              <h4 className="font-semibold text-base mb-1 line-clamp-2">
                {marketInfo.question ?? dict.trading?.orders?.market ?? "Market"}
              </h4>
              {getOutcome() && (
            ...
            <div className="text-sm text-gray-400">{dict.trading?.orders?.loading ?? "Loading..."}</div>
          )}
        </div>
      ...
      <div className="grid grid-cols-3 gap-3 p-3 bg-white/5 rounded-lg">
        <StatDisplay label={dict.trading?.orders?.price ?? "Price"} value={formatPrice(price)} />
        <StatDisplay label={dict.trading?.orders?.shares ?? "Shares"} value={formatShares(shares, 0)} />
        <StatDisplay label={dict.trading?.orders?.total ?? "Total"} value={formatCurrency(totalValue)} />
      </div>

      {/* Order ID */}
      <div className="mt-3 p-2 bg-white/5 rounded text-xs font-mono">
        <span className="text-gray-400">{dict.trading?.orders?.id ?? "ID:"}</span> {order.id.slice(0, 16)}...
      </div>
      ...
      >
        {isCancelling ? dict.trading?.orders?.cancelling ?? "Cancelling..." : dict.trading?.orders?.cancelOrder ?? "Cancel Order"}
      </button>
    </Card>
  );
}
